...
import (
	...
	templatehandler "github.com/OpenNSW/nsw/oga/internal/template/handler"
	templatemodel "github.com/OpenNSW/nsw/oga/internal/template/model"
)

type TemplateRegistry struct {
	templates         map[string]templatemodel.TemplateDefinition
	handlers          map[templatemodel.TemplateType]templatehandler.TemplateHandler
	defaultTemplateID string
}

func NewTemplateRegistry(dir string, defaultTemplateID string) (*TemplateRegistry, error) {
	registry := &TemplateRegistry{
		templates:         make(map[string]templatemodel.TemplateDefinition),
		handlers:          make(map[templatemodel.TemplateType]templatehandler.TemplateHandler),
		defaultTemplateID: defaultTemplateID,
	}

	if err := registry.RegisterHandler(templatehandler.NewFormHandler()); err != nil {
		return nil, err
	}
...
func (r *TemplateRegistry) RegisterHandler(h templatehandler.TemplateHandler) error {
	if h == nil {
...
func (r *TemplateRegistry) GetTemplate(id string) (templatemodel.TemplateDefinition, error) {
	templateDef, ok := r.templates[id]
	if !ok {
		return templatemodel.TemplateDefinition{}, fmt.Errorf("template %q not found", id)
	}

	return templateDef, nil
}

func (r *TemplateRegistry) GetDefaultTemplate() (templatemodel.TemplateDefinition, error) {
	if r.defaultTemplateID == "" {
		return templatemodel.TemplateDefinition{}, fmt.Errorf("no default template configured")
	}
...
func (r *TemplateRegistry) loadTemplates(dir string) error {
	...
	for _, entry := range entries {
		...
		var templateDef templatemodel.TemplateDefinition
		...
		slog.Info("loaded template", "id", templateID)
	}

	return nil
}

func (r *TemplateRegistry) validateTemplate(templateID string, templateDef templatemodel.TemplateDefinition) error {
	if err := r.validateSection(templateID, "roView", templateDef.ROView); err != nil {
		return err
	}

	if err := r.validateSection(templateID, "officerInput", templateDef.OfficerAction); err != nil {
		return err
	}
...
func (r *TemplateRegistry) validateSection(templateID string, sectionName string, section templatemodel.TemplateSection) error {
	if section.Type == "" {
