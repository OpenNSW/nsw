...
func toDomainStatus(s gateways.WebhookStatus) (PaymentStatus, error) {
	switch s {
	case gateways.WebhookStatusPending:
		return PaymentStatusPending, nil
	case gateways.WebhookStatusSuccess:
		return PaymentStatusSuccess, nil
	case gateways.WebhookStatusFailed:
		return PaymentStatusFailed, nil
	default:
		return "", fmt.Errorf("status %q: %w", s, gateways.ErrUnsupportedWebhookStatus)
	}
}
