...
func (m *mockRegistry) Get(string) (gateways.PaymentGateway, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
	return m.gw, nil
...
func TestCreateCheckoutSession_Success(t *testing.T) {
	...
	gw.On("CreateSession", mock.Anything, mock.Anything).
		...
		Return(&gateways.SessionResponse{
			SessionID:    "sess-1",
			Type:         gateways.FlowTypeInstruction,
	...
	assert.Equal(t, gateways.FlowTypeInstruction, resp.Type)
...
func TestCreateCheckoutSession_ReferenceCollisionRetry(t *testing.T) {
	...
	gw := new(MockGateway)
	gw.On("CreateSession", mock.Anything, mock.Anything).Return(&gateways.SessionResponse{}, nil)
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
...
func TestValidateReference_PayablePending(t *testing.T) {
	...
	gw.On("HandleValidateReference", mock.Anything,
		mock.MatchedBy(func(tx *gateways.ValidationTransaction) bool { return tx != nil && tx.ReferenceNumber == "TNSW1" }),
		true, mock.Anything).
		Return(&gateways.ValidationResponse{HTTPStatus: 200, Payload: []byte(`{}`)}, nil)
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
...
func TestValidateReference_UnknownReference(t *testing.T) {
	...
	gw.On("HandleValidateReference", mock.Anything,
		mock.MatchedBy(func(tx *gateways.ValidationTransaction) bool { return tx == nil }),
		false, mock.Anything).
		Return(&gateways.ValidationResponse{HTTPStatus: 200, Payload: []byte(`{}`)}, nil)
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
...
func webhookGateway(p *gateways.WebhookPayload) *MockGateway {
	gw := new(MockGateway)
...
func TestProcessWebhook_SuccessAdvancesTask(t *testing.T) {
	...
	gw := webhookGateway(&gateways.WebhookPayload{
		ReferenceNumber:      "TNSW1",
		Status:               gateways.WebhookStatusSuccess,
		Amount:               decimal.RequireFromString("1500.00"),
		Currency:             "LKR",
...
func TestProcessWebhook_Failed(t *testing.T) {
	...
	gw := webhookGateway(&gateways.WebhookPayload{
		ReferenceNumber: "TNSW1",
		Status:          gateways.WebhookStatusFailed,
	})
	tc := &mockTaskCompleter{}
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
	svc.SetTaskCompleter(tc)
...
func TestProcessWebhook_NotFound(t *testing.T) {
	repo := newMockRepo()
	gw := webhookGateway(&gateways.WebhookPayload{ReferenceNumber: "NOPE", Status: gateways.WebhookStatusSuccess})
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
...
func TestProcessWebhook_UnsupportedStatus(t *testing.T) {
	...
	gw := webhookGateway(&gateways.WebhookPayload{
		ReferenceNumber: "TNSW1",
		Status:          gateways.WebhookStatus("WEIRD"),
	})
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})

	err := svc.ProcessWebhook(context.Background(), "govpay", []byte(`{}`), nil)
	require.ErrorIs(t, err, gateways.ErrUnsupportedWebhookStatus)
	assert.Equal(t, PaymentStatusPending, repo.txs["TNSW1"].Status)
}
...
func TestProcessWebhook_NonTerminalDoesNotAdvance(t *testing.T) {
	...
	gw := webhookGateway(&gateways.WebhookPayload{
		ReferenceNumber: "TNSW1",
		Status:          gateways.WebhookStatusPending,
	})
	tc := &mockTaskCompleter{}
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
	svc.SetTaskCompleter(tc)
...
func TestCreateCheckoutSession_SessionIDPersistError(t *testing.T) {
	...
	gw := new(MockGateway)
	gw.On("CreateSession", mock.Anything, mock.Anything).
		Return(&gateways.SessionResponse{SessionID: "sess-1"}, nil)
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
...
func TestProcessWebhook_RepoGetError(t *testing.T) {
	repo := newMockRepo()
	repo.getErr = errors.New("db down")
	gw := webhookGateway(&gateways.WebhookPayload{ReferenceNumber: "TNSW1", Status: gateways.WebhookStatusSuccess})
	svc := NewPaymentService(repo, &mockRegistry{gw: gw})
