...
type mockService struct {
	validateResp *gateways.ValidationResponse
	validateErr  error
	webhookErr   error
}
...
func (m *mockService) ValidateReference(context.Context, string, json.RawMessage) (*gateways.ValidationResponse, error) {
...
func TestHandleWebhook_StatusClassification(t *testing.T) {
	cases := map[string]struct {
		...
		"unsupported status": {err: fmt.Errorf("bad: %w", gateways.ErrUnsupportedWebhookStatus), want: http.StatusBadRequest},
...
func TestHandleValidateReference_WritesGatewayResponse(t *testing.T) {
	svc := &mockService{validateResp: &gateways.ValidationResponse{
