...
func (s *ConsignmentService) GetConsignmentsByTraderID(ctx context.Context, traderID string) ([]r_model.ConsignmentResponseDTO, error) {
	...
	if len(consignments) == 0 {
		return []r_model.ConsignmentResponseDTO{}, nil
	}
	...
	nodesByConsignment := make(map[uuid.UUID][]r_model.WorkflowNode)
...
func (s *ConsignmentService) updateWorkflowNodeStateAndPropagateChangesInTx(ctx context.Context, tx *gorm.DB, updateReq *r_model.UpdateWorkflowNodeDTO) ([]r_model.WorkflowNode, map[string]any, error) {
	...
	case r_model.WorkflowNodeStateFailed:
		if workflowNode.State != r_model.WorkflowNodeStateFailed {
	...
	case r_model.WorkflowNodeStateCompleted:
		if workflowNode.State != r_model.WorkflowNodeStateCompleted {
