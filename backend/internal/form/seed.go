...
func SeedForms(db *gorm.DB, examplesDir string) error {
	exampleFiles := []string{
		...
		"tea_permit.json",
		"oga_review.json",
		"cda_salmonella_certificate.json",
		"health_certificate.json",
		"phytosanitary_certificate.json",
		"coconut_cess_payment.json",
	}
	...
	for _, filename := range exampleFiles {
		...
		var formData struct {
			...
			VerificationSchema   json.RawMessage `json:"verificationSchema"`
			VerificationUISchema json.RawMessage `json:"verificationUiSchema"`
		}
		...
		form := model.Form{
			...
			VerificationSchema:   formData.VerificationSchema,
			VerificationUISchema: formData.VerificationUISchema,
			Active:               true,
		}
