...
function propertyToZod(property: JsonSchema, required: boolean): ZodTypeAny {
    ...
    switch (type) {
        case 'string': {
            ...
            if (property.enum && property.enum.length > 0) {
                const enumValues = property.enum.filter((v: unknown): v is string => typeof v === 'string');
        ...
        case 'integer': {
            ...
            if (property.exclusiveMinimum !== undefined && typeof property.exclusiveMinimum === 'number') {
                schema = schema.gt(property.exclusiveMinimum, `Value must be greater than ${property.exclusiveMinimum}`);
            }

            if (property.exclusiveMaximum !== undefined && typeof property.exclusiveMaximum === 'number') {
...
export function schemaToZod(jsonSchema: JsonSchema): z.ZodObject<Record<string, ZodTypeAny>> {
    ...
    if (jsonSchema.properties) {
        for (const [name, property] of Object.entries(jsonSchema.properties)) {
            shape[name] = propertyToZod(property as JsonSchema, requiredFields.has(name));
        }
    }
